...
{
	public $module_id=WF_PKLIST_POST_TYPE.'_customizer';
	...
	{
		add_action('wp_ajax_wfpklist_customizer_get_template_data',array($this,'get_template_data'),1);
		add_action('wp_ajax_wfpklist_customizer_update_from_codeview',array($this,'update_from_codeview'),1);
		add_action('wp_ajax_wfpklist_customizer_save_theme',array($this,'save_theme'),1);
		add_action('wp_ajax_wfpklist_customizer_my_templates',array($this,'my_templates'),1);
	}
	...
	public function out_settings_form($args)
	{
		...
		Wf_Woocommerce_Packing_List_Admin::envelope_settings_subtabcontent(WF_PKLIST_POST_TYPE.'-customize',$view_file,'',$params,0);
	}
	...
	protected function load_template_header_footer($path,$template,$page_title="")
	{
		include $path;
	...
	*/
	public function my_templates()
	{
		...
		if($template_type!="")
		{
			$template_action=isset($_POST['template_action']) ? $_POST['template_action'] : '';
			if($template_action!="")
			{
				$template_id=isset($_POST['template_id']) ? $_POST['template_id']*1 : 0;
	...
	*/
	public function save_theme()
	{
		...
		$template_id=isset($_POST['template_id']) ? $_POST['template_id']*1 : 0;
		$template_html=isset($_POST['codeview_html']) ? $_POST['codeview_html'] : '';
		$template_type=isset($_POST['template_type']) ? $_POST['template_type'] : '';
		$tme=time();
		if($template_id==0) //template id=0 then new theme
		{
			$def_template=isset($_POST['def_template']) ? $_POST['def_template']*1 : 0;
			$name=isset($_POST['name']) ? $_POST['name'] : date('d-m-Y h:i:s A');
	...
	*/
	...
	{
		$html=stripcslashes($_POST['codeview_html']);
		$template_type=isset($_POST['template_type']) ? $_POST['template_type'] : '';
		$out=array(
			'status'=>0,
	...
	*/
	public function append_header_and_footer_html($html,$template_type,$page_title)
	{
		...
		$header_html=$this->load_template_header_footer($template_path,'header',$page_title);
		$footer_html=$this->load_template_header_footer($template_path,'footer');
	...
	public function generate_template_html($html,$template_type,$order,$box_packing=null,$order_package=null)
	{
		...
		$find_replace=apply_filters('wf_pklist_alter_find_replace',$find_replace,$template_type,$order,$box_packing,$order_package);
